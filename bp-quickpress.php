...
//REQUIRED
require_once ( BP_QUICKPRESS_PLUGIN_DIR . '/bp-quickpress-templatetags.php' );
if (!class_exists('TreeSet'))
	require_once ( BP_QUICKPRESS_PLUGIN_DIR . '/bp-quickpress-classes.php' );
...
function bp_quickpress_setup_nav() {
	...
	bp_core_new_subnav_item( array( 'name' => __( 'QuickPress' ), 'slug' => __( 'quickpress', 'bp-quickpress-slugs' ), 'parent_url' => $blogs_link, 'parent_slug' => $bp->blogs->slug, 'screen_function' => 'bp_quickpress_screen_write_a_post', 'position' => 15 ) );
...
function bp_quickpress_scripts() {
	global $bp;
	if (( $bp->current_component == $bp->blogs->slug ) && ($bp->current_action==__( 'quickpress', 'bp-quickpress-slugs' ))) {
	wp_enqueue_script( 'bp-quickpress-expandable-tree-js', apply_filters('bp_quickpress_enqueue_url',get_stylesheet_directory_uri() . '/quickpress/_inc/js/expandableTree.js'),array('jquery'), '1.0' );
...
function bp_quickpress_css() {
	...
	if (( $bp->current_component == $bp->blogs->slug ) && ($bp->current_action==__( 'quickpress', 'bp-quickpress-slugs' )))
		wp_enqueue_style( 'bp-quickpress-screen', apply_filters('bp_quickpress_enqueue_url',get_stylesheet_directory_uri() . '/quickpress/style.css') );
...
function bp_quickpress_located_template( $located_template, $template_name ) {
	...
	if (!(( $bp->current_component == $bp->blogs->slug ) && ($bp->current_action==__( 'quickpress', 'bp-quickpress-slugs' )))) return false;
...
?>
	<div id="message" class="error">
		<p><?php echo apply_filters('bp_quickpress_no_capability_msg',__( 'You do not have the capability to do this', 'bp-quickpress' )); ?></p>
	</div>
<?php
}
...
function bp_quickpress_screen_edit_a_post() {
	...
	if (!(( $bp->current_component == $bp->blogs->slug ) && ($bp->action_variables[0]==__( 'edit', 'bp-quickpress-slugs' )))) return false;
	...
	if ((!$quickpress_post) || (($is_author) && ((!bp_quickpress_user_can('edit_posts')))) || ((!$is_author) && ((!bp_quickpress_user_can('edit_others_posts'))))) {
		$creation_url = $bp->loggedin_user->domain . $bp->blogs->slug . '/'.__( 'quickpress', 'bp-quickpress-slugs' ).'/';
...
function bp_quickpress_screen_save($creation_url=false,$edition_url=false,$published_url=false,$redirect=true) {
	...
	if ($bp->action_variables[0]!=__( 'save', 'bp-quickpress-slugs' )) return false;
	
	if (!bp_quickpress_user_can('edit_posts')) {
		bp_core_add_message(__( 'You do not have the capability to do this', 'bp-quickpress' ));
	}
	...
	if (!$edition_url) 
		$edition_url = $bp->loggedin_user->domain . $bp->blogs->slug . '/'.__( 'quickpress', 'bp-quickpress-slugs' ).'/'.__( 'edit', 'bp-quickpress-slugs' ).'/';
		
	check_admin_referer( 'bp-quickpress-new-post' );
	...
	if ((!$post_content) || (!$post_title))  {
		bp_core_add_message(__( 'Please fill the required fields (title,description).', 'bp-quickpress' ));
		bp_core_redirect($creation_url);				
	}
	...
	if (!$post_id) {
		...
		if (!$post['ID']) {
			bp_core_add_message( __( 'Post draft saved !  Please check if everything is okay then publish it !','bp-quickpress' ));
			...
			if ($post['post_status']!='publish') {
				bp_core_add_message( __( 'Your post has been saved, now wait for the administrator to publish it.','bp-quickpress' ));
			}else {
				bp_core_add_message( __('Post published.'));
			}
			if ($redirect) {
				if (!$published_url) {
